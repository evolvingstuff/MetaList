...
- **Hosted validation pending:** push the final candidate commit, then require every Windows/macOS/Linux × Python 3.10–3.13 job in `Publish to PyPI` to succeed for that exact SHA before creating a release tag. Earlier successful workflow runs do not validate 0.5.0. Local results do not establish all-platform compatibility; no tag or publication is authorized by this record.
