...
The HTTPS transport retains separate listeners and sends response headers and body chunks immediately. Browser HTTP/1.1 connections remain reusable across module and API requests; explicit client closure and failed transfers close the connection. Its pending accept queue holds 32 connections, matching the maximum 32 active workers. It uses 64 KiB chunks, a 60-second socket idle timeout, and a 30-minute total request deadline checked between transfers. Disconnects close the upstream connection when observed during a read/write or by timeout. Chunked incoming bodies are explicitly rejected with 501; ambiguous Content-Length/Transfer-Encoding and duplicated Host headers return 400. Hop-by-hop headers are stripped in both directions, including fields nominated by Connection. Forwarded peer headers are replaced with the actual client address.
...
### F15 password and stream boundaries (2026-09-12)
