...
- A single categorical preference controls `existing tags only` versus `allow new tags`. Track whether the user has explicitly chosen it. Every generation request asks through one structured existing-only/new-only/both selector and updates this permission from the submitted choice. Request wording and model interpretation can never bypass this choice.
- The required selector resolves the current pass's focus: missing existing tags, new tags where existing vocabulary falls short, or both (favoring existing tags). Remember the exact last selector choice and preselect it on the next request while leaving all three choices available. Even an explicit request for existing, new, or both must pass through this confirmation.
...
- Provide an editable tagging prompt through the existing prompt menu. Treat a user-specified topic as a binding semantic filter: use examples to disambiguate the intended field, prefer specific concepts/methods/entities, omit unrelated notes, and reject broad neighboring classifications. Initial guidance also favors parent tags describing a group and child tags describing specific children, avoiding redundant inherited classifications. Visible children need not be exhaustive.
...
- Automated validation: 1,127 Python tests and 575 JavaScript tests passed; Python and JavaScript startup sanity gates passed. An isolated browser preview verified the categorical preference dialog and disabled Proceed before selection. Synthetic coverage includes a mandatory remembered three-way focus choice for every generation request, binding semantic topic instructions, valid cancellation activity rendering, tag-result combined-view links, 2,000 roots, later-batch failure/cancellation, bounded correction retries, unchanged history on no-ops, exact bulk filters, programmatic removal without a progress panel, unseen-sibling inheritance, and root-only versus recursive collapse behavior. Live model quality and full application interaction still require user testing. The latest completed checkpoint is `475afc67`; this follow-up fix remains uncommitted pending user testing.
